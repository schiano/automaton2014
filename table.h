 ...
 * 
 * Un table est une structure de donnée qui asscoie une clé à une et une seule
 * valeur.
 *
 * Une table ne peut pas contenir deux fois la même clé.
 * Par contre, à deux clés différente, on peut assicer deux fois la même valeur.
 */
 ...
 * plus compliquées.
 * 
 * La mémoire associée à la clée est gérée par la table, alors que celle
 * de la valeur est laissé à la charge de l'utilisateur.
 ...
 * mettre tous les patamètres 'comparer_cle', 'copier_cle' et 'supprimer_cle' à 
 ...
 *  - créée une table qui, à un entier, associe un entier.
 ...
 *                        Exemple 2
 *=============================================================================
 * Dans cette exemple, nous montrons comment associer un entier à un couple
 *
 *
 * typdef struct { int x; int y; } Couple;   // On définit le couple
 *
 * Table * t = creer_table( NULL; NULL, NULL );  // On créé la table
 *
 ...
 *                        Exemple 3
 *============================================================================= 
 * Voici maintenant un exemple de table ou l'on associe un couple à un entier
 *
 *
 ...
 * typdef struct { int x; int y; } Couple;   // On définit le couple
 *
 * Table * table = creer_table(           // On créé la table
...
/*
 * Cette fonction détruit une table. La mémoirei qui a été allouée par la table 
 * pour stocker les clés sont aussi détruitres. 
 * Attention, la table ne gère pas la mémoire associée aux valeurs.
 * C'est à l'utilisateur de détruire porement les valeurs.
 *
 */
 ...
 * Pour cela, add_table(), créé une copie de la clé passée en paramètre, 
 * l'enregistre dans la table et l'associe à la valeur passé en paramètre.
 * La mémoire de la copie de la clé est gérée par la table.
 * La mémoire de la clé passé en paramètre reste à la charge de l'utilisateur.
 * 
 ...
 * N'est PAS libérée ! Sa gestion est laissée à la charge de l'utilisateur.
 *
 */
...
/*
 * La fonction passe en revue toutes les valeurs de la table passé en param_tre
 * et apelle la fonction passé en paramètre pour chasune des valeur parcourues.
 ...
 *
 * La fonction qui sera executée (et qui a été passé en paramètre), doit 
 ...
 * et apelle la fonction passé en paramètre pour chasune de ces associations.
 *
 * La fonction qui sera executée (et qui a été passé en paramètre), doit 
 * posséder l'entête suivante :
 *     void NON_FONCTION( const intptr_t cle, intptr_t valeur, void* data  ),
 ...
 * (pour la fonction de comparaison de clé de la table) à la clé passé en 
 * paramètre.
 */
 ...
 *
 * C'est à dire, l'iterateur renvoyé est positionné sur une nouvelle 
 * association qui est la plus petite des associations plus grandes que celle
 * associée à l'iterateur passé en en paramètre.
 *
 * Deux associations sont comparés en comparant leurs clés et la fonction de 
 * comparaison des clés de la table.
 *
...
/*
 * Renvoie la clé de l'association pointé par l'itérateur passé en paramètre..
 */
const intptr_t get_cle( Table_iterateur it );
/*
 * Renvoie la valeur de l'association pointé par l'itérateur passé en paramètre.
 */
